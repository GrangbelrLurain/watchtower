...
import { Activity, AlertCircle, ArrowLeft, BarChart3, Clock, Globe, ShieldCheck } from "lucide-react";
          ...
          <ul className="space-y-3">
            <li className="flex justify-between text-xs border-b border-slate-50 pb-2">
              ...
              <span className="font-mono font-bold text-slate-700">
                {logs.length > 0 ? Math.max(...logs.map((l) => l.latency)) : 0}ms
              </span>
            </li>
            <li className="flex justify-between text-xs border-b border-slate-50 pb-2">
              <span className="text-slate-500">Min Latency</span>
              <span className="font-mono font-bold text-slate-700">
                {logs.length > 0 ? Math.min(...logs.map((l) => l.latency)) : 0}ms
              </span>
            </li>
            <li className="flex justify-between text-xs border-b border-slate-50 pb-2">
              ...
              {logs.map((log, i) => (
                <tr key={i} className="hover:bg-slate-50/50 transition-colors">
                  ...
                  <td className="px-6 py-3">
                    <Badge variant={{ color: log.ok ? "green" : "red", size: "sm" }}>{log.status}</Badge>
                  </td>
                  <td className="px-6 py-3 text-xs font-mono text-slate-600">{log.latency}ms</td>
                  <td className="px-6 py-3 text-xs text-slate-400 italic">{log.errorMessage ?? "-"}</td>
                </tr>
              ))}
