...
function domainToHost(domain: string): string {
  const d = domain.trim();
  const after = d.startsWith("https://")
    ? d.slice(8)
    : d.startsWith("http://")
      ? d.slice(7)
      : d;
...
function ProxySetupPage() {
  ...
  const pacUrl =
    port > 0 ? `http://127.0.0.1:${port}/.watchtower/proxy.pac` : "";
      ...
      {!proxyStatus.running ? (
        <Card className="p-6">
          <P className="text-slate-600">
            Proxy is not running. Start the proxy from the Proxy page first.
          </P>
          <Link to="/proxy">
        ...
        <div className="flex flex-col gap-8">
          ...
          <Card className="p-6">
            ...
            <P className="text-slate-600 mb-3">
              Enter the URL below in your system or browser's automatic proxy
              configuration.
            </P>
            <div className="bg-slate-100 rounded-lg p-4 font-mono text-sm break-all">
              {pacUrl || "—"}
            </div>
            <P className="text-slate-500 text-sm mt-3">
              Windows: Settings → Network & Internet → Proxy → Use setup script
              <br />
              macOS: System Settings → Network → Advanced → Proxies → Automatic
          ...
          <Card className="p-6">
            ...
            <P className="text-slate-600 mb-2">
              To use manual proxy instead of PAC:
            </P>
            <ul className="list-disc list-inside text-slate-600 space-y-1">
              <li>
                Address:{" "}
                <code className="bg-slate-100 px-1 rounded">127.0.0.1</code>
              </li>
              <li>
          ...
          <Card className="p-6">
            ...
            <P className="text-slate-600 mb-3">
              To trust the certificate for local HTTPS, download the certificate
              for each host and install it on your system. (Links open in the
              browser.)
            </P>
            {hosts.length === 0 ? (
              <P className="text-slate-500 text-sm">
                No enabled local routes. Add and enable domains on the Proxy
                page.
              </P>
            ) : (
            ...
            <P className="text-slate-500 text-sm mt-4">
              Windows: Double-click .crt → Install certificate → Local computer
              → Trusted root certification authorities. (If Open with dialog
              appears: run certmgr.msc → Import)
              <br />
