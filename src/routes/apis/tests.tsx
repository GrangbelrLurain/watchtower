...
function ApiTestsPage() {
  ...
  const [results, setResults] = useState<
    Record<string, { running: boolean; ok?: boolean; status?: number; msg?: string }>
  >({});
  ...
  const handleRemove = async (id: string) => {
    if (!confirm("Remove this test case?")) {
      return;
    }
    try {
  ...
  const filteredTests = tests.filter(
    (t) => t.name.toLowerCase().includes(search.toLowerCase()) || t.url.toLowerCase().includes(search.toLowerCase()),
  );
          ...
          <div className="space-y-4">
            {filteredTests.map((test) => {
              ...
              return (
                <div
                  key={test.id}
                  className="p-4 rounded-xl border border-slate-100 flex items-center gap-4 hover:border-emerald-100 transition-colors"
                >
                  ...
                  {res && (
                    <div
                      className={`px-3 py-1 rounded-lg text-xs font-bold ${res.running ? "bg-slate-100 animate-pulse" : res.ok ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"}`}
                    >
                      {res.running
                        ? "Running..."
                        : res.ok
